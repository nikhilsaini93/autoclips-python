...
2. `POST /clip` — cut a clip from a YouTube video between `start` and `end`, with an optional 9:16 face-aware crop and burned-in subtitles (`none` | `english` | `native` | `hinglish`)
...
curl -X POST http://localhost:8000/clip \
  ...
  -o clip.mp4
```
`subtitles` is `none` | `english` (translated) | `native` (as spoken, e.g. Hindi) | `hinglish` (as spoken, Latin script — Hindi audio reads as Hinglish, English passes through). Hinglish reuses the same transcript as clip detection, so timing aligns exactly.
